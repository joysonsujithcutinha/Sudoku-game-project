document.addEventListener('DOMContentLoaded', () => {
    ...
    const puzzleTypeSelect = document.getElementById('puzzle');
    const clearButton = document.getElementById('clear');
    const solveButton = document.getElementById('solve')
 
    // Initialize the grid based on the initial puzzle type
    createGrid();

    // Listen for changes on the puzzle type dropdown
    puzzleTypeSelect.addEventListener('change', createGrid);
    ...
    function solve(){
        ...
        // Get the array of entered puzzle
        // Get input array - '' or '1'/'2' -> 0 or 1/2
        //             inputpuzzle = [] <- append values foreach loop
        let inpuzzle = [];

        // Assuming `cells` is an array of cell elements
        cells.forEach(cell => {
            if (cell.textContent === '') {
                inpuzzle.push(0);
            }
            // else if(cell.textContent === '1'/'2'){
            //   inpuzzle.push(1/2);
            // } 
            else {
        ...
        if (solveSudoku(grid2D,cells)) {
            // printGrid(grid2D);
            // console.log("Output 1D:", convert2DTo1D(grid2D));
            let outgrid = convert2DTo1D(grid2D);
            //alert('solution found');
            //console.log(outgrid);
            // for(let i = 0; i< outgrid.length; i++) {
            //     cells[i].textContent = String(outgrid[i]);
            // }
        } else {
        ...
        async function solveSudoku(grid,cells) {
            ...
            // size gives length
            // 2d to 1d index conversion: row*colsize+col
            ...
            for (let num = 1; num <= size; num++) {
                if (isSafe(grid, row, col, num)) {
                    ...
                    await sleep(1);
                    // console.log(`Filled (${row}, ${col}) with ${num}`);
                    //printGrid(grid); // Print current grid state
                    //console.log();
    ...
    // Create the grid based on selected puzzle type
    function createGrid() {
        const puzzleType = puzzleTypeSelect.value;
        document.body.setAttribute('puzzletype', puzzleType);
        const [gridSize, rowSize, colSize] = getSizes();
        ...
        applyStyles(gridSize, rowSize, colSize);
        grid.addEventListener('input', handleInput);
        grid.addEventListener('keydown', handleKeyDown);
    }

    // Function to apply styles to the grid
    function applyStyles(gridSize, rowSize, colSize) {
        const cells = grid.querySelectorAll('td');
